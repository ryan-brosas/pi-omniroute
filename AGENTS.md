...
| `.github/` | quality.yml gates (check/test/typecheck + required job), release.yml (stable `v*` + per-push continuous GitHub Releases), npm-publish.yml (on-demand npm publish + GitHub Release skip-if-exists, workflow_dispatch), pr-quality.yml anti-slop gate, dependabot (actions + npm), PR/issue templates |
...
- **Right-size the catalog.** `OMNIROUTE_MODEL_SCOPE` controls what registers at all: `active` (default — live ids backed by an active gateway connection via dashboard `/api/providers`, plus the `auto` router; degrades to the auto router alone when the dashboard API is unavailable), `curated` (verified floor, fully offline), `routes` (auto router + `auto/*`), or `all` (full live catalog; live fetch, tombstones, and the persisted store apply only here). In the `all` scope the registered catalog is capped at 1000 (`MAX_REGISTERED_MODELS` in `models.ts`, priority: auto → customs → live → tombstones); the live fetch is additionally trimmed by `MAX_LIVE_CATALOG_ENTRIES`. The disk cache is keyed to the configured base URL — switching `OMNIROUTE_BASE_URL` starts a fresh catalog.
