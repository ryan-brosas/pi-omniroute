...
| *(offline fallback: 37 curated models — `auto` + all built-in `auto/*` routes (`auto/best-coding`, `auto/reasoning:pro`, `auto/vision`, …) + `claude/claude-sonnet-4-6`, `cc/claude-opus-4-6`, `glm/glm-5.2`, …)* | The curated floor is part of every scope. `/v1/models` exposes no credential/availability info, so ids without backing credentials on your OmniRoute instance can still error at request time — keyless setups should prefer the `auto` routes |
...
| `OMNIROUTE_CACHE_DIR` | `<agent dir>/cache` | Where the delisted-model tombstone store is kept (the model catalog itself is cached by pi). Keyed to the configured gateway — changing `OMNIROUTE_BASE_URL` starts a fresh tombstone history. Tombstones apply only in the `all` scope |
| `OMNIROUTE_MODEL_SCOPE` | `active` | Registered catalog: `active` (default) — live ids backed by an active gateway connection (dashboard `/api/providers`), unioned with the curated floor; `curated` — the verified floor only, fully offline; `routes` — only the auto router + `auto/*`; `all` — the full live `/v1/models` catalog. `active` degrades to the curated floor when the dashboard API is unavailable |
...
2. **Platform-native refresh** — the extension implements `ProviderConfig.refreshModels`: pi owns the cadence, the persisted catalog store (`publish({persist})`), and offline/cache-only phases (`allowNetwork`). In the default `active` scope, each refresh layers live `/v1/models` → curated fields → `patch.json` → `custom-models.json` and keeps only the ids your active gateway connections back (plus the `auto` routes and the curated floor) — the dashboard's active-connection list (`/api/providers`) drives the cut, degrading to the curated floor when it's unavailable. The `all` scope additionally layers tombstoned ids (14-day grace). Live discovery runs on interactive sessions (session start and `/model`); headless `pi -p` sessions serve the stored or curated catalog without network. The static `curated`/`routes` scopes never fetch.
