...
| *(offline fallback: 37 curated models — `auto` + all built-in `auto/*` routes (`auto/best-coding`, `auto/reasoning:pro`, `auto/vision`, …) + `claude/claude-sonnet-4-6`, `cc/claude-opus-4-6`, `glm/glm-5.2`, …)* | The default `curated` scope registers this verified floor. It is **not** filtered by what your gateway can serve — `/v1/models` exposes no credential/availability info; ids without backing credentials on your OmniRoute instance error at request time, so keyless setups should prefer the `auto` routes |
...
| `OMNIROUTE_MODEL_SCOPE` | `curated` | Registered catalog: `curated` — the verified floor (`models.json` + `custom-models.json` + patches, fully offline); `routes` — only the auto router + `auto/*`; `all` — the full live `/v1/models` catalog |
...
2. **Platform-native refresh** — the extension implements `ProviderConfig.refreshModels`: pi owns the cadence, the persisted catalog store (`publish({persist})`), and offline/cache-only phases (`allowNetwork`). In the `all` scope, each refresh layers live `/v1/models` → curated fields → `patch.json` → `custom-models.json` → tombstoned ids (14-day grace) and hot-swaps the registration; live discovery runs on interactive sessions (session start and `/model`), while headless `pi -p` sessions serve the stored or curated catalog without network. The default `curated` scope is fully offline: it registers the verified floor and never fetches.
