...
| `OMNIROUTE_MODEL_SCOPE` | `active` | Registered catalog: `active` (default) — live ids backed by an active gateway connection (dashboard `/api/providers`), plus the `auto` router; degrades to the auto router alone when the dashboard API is unavailable. `curated` — the verified floor only, fully offline; `routes` — only the auto router + `auto/*`; `all` — the full live `/v1/models` catalog |
...
2. **Platform-native refresh** — the extension implements `ProviderConfig.refreshModels`: pi owns the cadence, the persisted catalog store (`publish({persist})`), and offline/cache-only phases (`allowNetwork`). In the default `active` scope, each refresh layers live `/v1/models` → curated fields → `patch.json` → `custom-models.json` and keeps only the ids your active gateway connections back (plus the `auto` router) — the dashboard's active-connection list (`/api/providers`) drives the cut, degrading to the auto router alone when it's unavailable. The `all` scope additionally layers tombstoned ids (14-day grace). Live discovery runs on interactive sessions (session start and `/model`); headless `pi -p` sessions serve the stored or curated catalog without network. The static `curated`/`routes` scopes never fetch.
